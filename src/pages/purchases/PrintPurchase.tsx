            ...
            <div className="mb-8">
                <table className="w-full border-collapse">
                    ...
                    <tbody>
                        {items.length === 0 ? (
                            <tr>
                                <td colSpan={purchase.is_vat_included ? 10 : 11} className="border p-2 pt-3 pb-3 text-center text-gray-500 text-xs">
                                    Tidak ada item
                                </td>
                            </tr>
                        ) : (
                            items.map((item, index) => (
                                <tr key={item.id} className="hover:bg-gray-50 text-xs">
                                    <td className="border p-1 pt-2 pb-2 text-center">{index + 1}</td>
                                    <td className="border p-1 pt-2 pb-2">{item.item?.code || '-'}</td>
                                    <td className="border p-1 pt-2 pb-2">{item.item?.name || 'Item tidak ditemukan'}</td>
                                    <td className="border p-1 pt-2 pb-2 text-center">{item.batch_no || '-'}</td>
                ...
                {subtotals && (
                    <div className="border p-4 min-w-[250px] text-sm">
                        <div className="grid grid-cols-[1fr,auto,1fr] mb-1">
                            ...
                            <span className="px-2">:</span>
                            <span>{subtotals.baseTotal.toLocaleString('id-ID')}</span>
                        </div>

                        <div className="grid grid-cols-[1fr,auto,1fr] mb-1">
                            <span className="text-left">Diskon</span>
                            <span className="px-2">:</span>
                            <span>-{subtotals.discountTotal.toLocaleString('id-ID')}</span>
                        </div>

                        <div className="grid grid-cols-[1fr,auto,1fr] mb-1">
                            <span className="text-left">Setelah Diskon</span>
                            <span className="px-2">:</span>
                            <span>{subtotals.afterDiscountTotal.toLocaleString('id-ID')}</span>
                        </div>
                        ...
                        {!purchase.is_vat_included && (
                            <div className="grid grid-cols-[1fr,auto,1fr] mb-1">
                                ...
                                <span className="px-2">:</span>
                                <span>+{subtotals.vatTotal.toLocaleString('id-ID')}</span>
                            </div>
                        )}
                        ...
                        <div className="border-t pt-2 grid grid-cols-[1fr,auto,1fr] font-bold">
                            ...
                            <span className="px-2">:</span>
                            <span>{subtotals.grandTotal.toLocaleString('id-ID')}</span>
                        </div>
                    </div>
                )}
            </div>
