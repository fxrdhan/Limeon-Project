/**
 * jsdiff - Exact Character Diff Implementation from jsdiff
 *
...
> {
  ...
  diff(
    ...
    options:
      | DiffCallbackNonabortable<ValueT>
      | (AllDiffOptions & Partial<CallbackOptionNonabortable<ValueT>>) = {}
  ): ChangeObject<ValueT>[] | undefined {
    let callback:
      | DiffCallbackAbortable<ValueT>
      | DiffCallbackNonabortable<ValueT>
      | undefined;
  ...
  private diffWithOptionsObj(
    ...
    newTokens: TokenT[],
    options: AllDiffOptions &
      Partial<TimeoutOption> &
      Partial<MaxEditLengthOption>,
    callback:
      | DiffCallbackAbortable<ValueT>
      | DiffCallbackNonabortable<ValueT>
      | undefined
  ...
  equals(left: TokenT, right: TokenT, options: AllDiffOptions): boolean {
    if (options.comparator) {
      ...
      return (
        left === right ||
        (!!options.ignoreCase &&
          (left as string).toLowerCase() === (right as string).toLowerCase())
      );
    }
  }
...
// Convert to our existing DiffSegment format for backward compatibility
export interface DiffSegment {
  type: 'unchanged' | 'added' | 'removed';
  text: string;
}

export function convertChangesToSegments(
