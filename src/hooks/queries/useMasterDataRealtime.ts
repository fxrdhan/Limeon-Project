...
import type { Category, MedicineType, Unit, Item, ItemManufacturer } from '@/types/database';
...
export const useCategoriesRealtime = (options?: UseMasterDataRealtimeOptions) => {
        ...
        (payload: { eventType: string; new?: unknown; old?: unknown }) => {
          ...
          switch (payload.eventType) {
            case 'INSERT': {
              ...
              const insertKeysToInvalidate = getInvalidationKeys.masterData.categories();
            ...
            case 'UPDATE': {
              ...
              const updateKeysToInvalidate = getInvalidationKeys.masterData.categories();
            ...
            case 'DELETE': {
              ...
              queryClient.removeQueries({
                queryKey: QueryKeys.masterData.categories.detail(deletedCategory.id)
              });
              
              const deleteKeysToInvalidate = getInvalidationKeys.masterData.categories();
  ...
  useEffect(() => {
    return () => {
      if (subscriptionRef.current) {
        console.log('🧹 Component unmounting - cleaning up categories realtime subscription');
        subscriptionRef.current.unsubscribe();
        subscriptionRef.current = null;
      }
    };
...
export const useMedicineTypesRealtime = (options?: UseMasterDataRealtimeOptions) => {
  const queryClient = useQueryClient();
  const subscriptionRef = useRef<ReturnType<typeof supabase.channel> | null>(null);
        ...
        (payload: { eventType: string; new?: unknown; old?: unknown }) => {
          ...
          switch (payload.eventType) {
            case 'INSERT': {
              ...
              const insertKeysToInvalidate = getInvalidationKeys.masterData.types();
              insertKeysToInvalidate.forEach((keySet: readonly string[]) => {
                queryClient.invalidateQueries({ queryKey: keySet });
              });
              break;
            ...
            case 'UPDATE': {
              ...
              const updateKeysToInvalidate = getInvalidationKeys.masterData.types();
              updateKeysToInvalidate.forEach((keySet: readonly string[]) => {
                queryClient.invalidateQueries({ queryKey: keySet });
              });
              break;
            ...
            case 'DELETE': {
              ...
              const deleteKeysToInvalidate = getInvalidationKeys.masterData.types();
              deleteKeysToInvalidate.forEach((keySet: readonly string[]) => {
                queryClient.invalidateQueries({ queryKey: keySet });
              });
              break;
  ...
  useEffect(() => {
    return () => {
      if (subscriptionRef.current) {
        console.log('🧹 Component unmounting - cleaning up types realtime subscription');
        subscriptionRef.current.unsubscribe();
        subscriptionRef.current = null;
      }
    };
...
export const useUnitsRealtime = (options?: UseMasterDataRealtimeOptions) => {
        ...
        (payload: { eventType: string; new?: unknown; old?: unknown }) => {
          ...
          switch (payload.eventType) {
            case 'INSERT': {
              ...
              const insertKeysToInvalidate = getInvalidationKeys.masterData.units();
              insertKeysToInvalidate.forEach((keySet: readonly string[]) => {
                queryClient.invalidateQueries({ queryKey: keySet });
              });
              break;
            ...
            case 'UPDATE': {
              ...
              const updateKeysToInvalidate = getInvalidationKeys.masterData.units();
              updateKeysToInvalidate.forEach((keySet: readonly string[]) => {
                queryClient.invalidateQueries({ queryKey: keySet });
              });
              break;
            ...
            case 'DELETE': {
              ...
              const deleteKeysToInvalidate = getInvalidationKeys.masterData.units();
              deleteKeysToInvalidate.forEach((keySet: readonly string[]) => {
                queryClient.invalidateQueries({ queryKey: keySet });
              });
              break;
  ...
  useEffect(() => {
    return () => {
      if (subscriptionRef.current) {
        console.log('🧹 Component unmounting - cleaning up units realtime subscription');
        subscriptionRef.current.unsubscribe();
        subscriptionRef.current = null;
      }
    };
...
export const useManufacturersRealtime = (options?: UseMasterDataRealtimeOptions) => {
  const queryClient = useQueryClient();
  const subscriptionRef = useRef<ReturnType<typeof supabase.channel> | null>(null);
        ...
        (payload: { eventType: string; new?: unknown; old?: unknown }) => {
          ...
          switch (payload.eventType) {
            case 'INSERT': {
              ...
              const insertKeysToInvalidate = getInvalidationKeys.masterData.manufacturers?.() || [['manufacturers']];
              insertKeysToInvalidate.forEach((keySet: readonly string[]) => {
                queryClient.invalidateQueries({ queryKey: keySet });
              });
              break;
            ...
            case 'UPDATE': {
              ...
              queryClient.setQueryData(
                QueryKeys.masterData.manufacturers?.detail(updatedManufacturer.id) || ['manufacturers', 'detail', updatedManufacturer.id], 
                updatedManufacturer
              );
              
              const updateKeysToInvalidate = getInvalidationKeys.masterData.manufacturers?.() || [['manufacturers']];
              updateKeysToInvalidate.forEach((keySet: readonly string[]) => {
                queryClient.invalidateQueries({ queryKey: keySet });
              });
              break;
            ...
            case 'DELETE': {
              ...
              queryClient.removeQueries({
                queryKey: QueryKeys.masterData.manufacturers?.detail(deletedManufacturer.id) || ['manufacturers', 'detail', deletedManufacturer.id]
              });
              
              const deleteKeysToInvalidate = getInvalidationKeys.masterData.manufacturers?.() || [['manufacturers']];
              deleteKeysToInvalidate.forEach((keySet: readonly string[]) => {
                queryClient.invalidateQueries({ queryKey: keySet });
              });
              break;
            ...
            default:
              console.log('🔄 Unknown manufacturer event type:', payload.eventType);
          }
        }
      )
      .subscribe((status) => {
  ...
  useEffect(() => {
    return () => {
      if (subscriptionRef.current) {
        console.log('🧹 Component unmounting - cleaning up manufacturers realtime subscription');
        subscriptionRef.current.unsubscribe();
        subscriptionRef.current = null;
      }
    };
