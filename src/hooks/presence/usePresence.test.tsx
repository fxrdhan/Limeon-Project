...
describe('usePresence', () => {
  ...
  it('hydrates the navbar roster from browser-active presence state', async () => {
    ...
    expect(mockRealtimeService.replaceChannel).toHaveBeenCalledWith(
      'browser-active:user-a',
      {
        config: {
          presence: {
            key: 'user-a',
  ...
  it('reconnects the roster channel after a channel error', async () => {
    ...
    expect(mockRealtimeService.replaceChannel).toHaveBeenLastCalledWith(
      'browser-active:user-a',
      {
        config: {
          presence: {
  ...
  it('ignores stale roster setup work when a newer subscription already owns the channel', async () => {
    ...
    sharedChannel.subscribe.mockImplementation(
      (callback?: (status: string) => void) => {
        hasSubscribed = true;
        rosterChannelStatusHandler = callback ?? null;
        callback?.('SUBSCRIBED');
