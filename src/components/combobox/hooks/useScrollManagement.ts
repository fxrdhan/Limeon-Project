...
const scrollEstimatedOptionToPinnedTop = (
  ...
  const targetScrollTop = Math.min(
    maxScrollTop,
    Math.max(0, index * COMBOBOX_CONSTANTS.OPTION_ESTIMATED_HEIGHT - padding)
  );
  container.scrollTop = targetScrollTop;
};
  ...
  const checkScroll = useCallback(() => {
    ...
    setScrollState({
      ...
      reachedBottom:
        ...
        ) < COMBOBOX_CONSTANTS.SCROLL_THRESHOLD,
      scrolledFromTop:
        container.scrollTop > COMBOBOX_CONSTANTS.SCROLL_THRESHOLD,
    });
  }, [optionsContainerRef]);
