...
export const useComboboxEffects = ({
  ...
  const handleMouseLeaveWithCloseIntent = useCallback(() => {
    ...
    if (hoverToOpen && isOpen) {
      leaveTimeoutRef.current = setTimeout(
        actualCloseCombobox,
        COMBOBOX_CONSTANTS.CLOSE_TIMEOUT
      );
    }
  }, [actualCloseCombobox, hoverToOpen, isOpen]);
  ...
  useEffect(() => {
    if (isOpen && filteredOptions.length > 0) {
      ...
      if (highlightedOption && buttonRef.current) {
        ...
        const maxTextWidth = buttonWidth - COMBOBOX_CONSTANTS.BUTTON_PADDING;
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const events = [
        ['scroll', calculateComboboxPosition, true],
        ['resize', calculateComboboxPosition, false],
  ...
  }, [filteredOptions, isOpen, applyOpenStyles, calculateComboboxPosition]);
