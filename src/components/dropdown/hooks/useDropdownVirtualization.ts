...
type UseDropdownVirtualizationOptions = {
  enabled: boolean;
  itemCount: number;
  resetKey: unknown;
...
export type DropdownVirtualItem = {
  index: number;
  start: number;
};
...
export const useDropdownVirtualization = ({
  enabled,
  itemCount,
  resetKey,
  scrollContainerRef,
}: UseDropdownVirtualizationOptions) => {
  ...
  const { offsets, totalSize } = useMemo(() => {
    ...
    for (let index = 0; index < itemCount; index += 1) {
      ...
      nextTotalSize +=
        ...
        DROPDOWN_CONSTANTS.OPTION_ESTIMATED_HEIGHT;
    }
  ...
  const virtualItems = useMemo<DropdownVirtualItem[]>(() => {
    if (!enabled) return [];

    const items: DropdownVirtualItem[] = [];
  ...
  const getScrollTargetForIndex = useCallback(
    (index: number): KeyboardScrollTarget | null => {
      ...
      const itemTop =
        offsets[index] ?? index * DROPDOWN_CONSTANTS.OPTION_ESTIMATED_HEIGHT;
      const itemHeight =
        measuredHeightsRef.current.get(index) ??
        DROPDOWN_CONSTANTS.OPTION_ESTIMATED_HEIGHT;
