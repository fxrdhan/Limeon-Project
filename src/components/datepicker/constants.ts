...
export const getYearsToDisplay = (year: number): number[] => {
  const startYear = Math.floor(year / DATEPICKER_CONSTANTS.DECADE_SHIFT) * DATEPICKER_CONSTANTS.DECADE_SHIFT;
  return Array.from({ length: DATEPICKER_CONSTANTS.YEARS_PER_DECADE }, (_, i) => startYear + i - 1);
};
