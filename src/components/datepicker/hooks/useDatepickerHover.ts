...
import type { UseDatepickerHoverParams, UseDatepickerHoverReturn } from "../types";

export const useDatepickerHover = (params: UseDatepickerHoverParams): UseDatepickerHoverReturn => {
  ...
  const handleTriggerMouseLeave = useCallback(() => {
    ...
    closeTimeoutRef.current = setTimeout(() => {
      if (
        portalRef.current &&
        !portalRef.current.matches(":hover")
      ) {
        closeCalendar();
      }
