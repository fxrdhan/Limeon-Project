...
import type { UseDatepickerKeyboardParams, UseDatepickerKeyboardReturn } from "../types";

export const useDatepickerKeyboard = (params: UseDatepickerKeyboardParams): UseDatepickerKeyboardReturn => {
  const {
    isOpen,
    currentView,
    highlightedDate,
  ...
  const handleDaysNavigation = useCallback((e: React.KeyboardEvent) => {
    ...
    if (navigated) {
      ...
      if (isValidDate) {
        ...
        if (
          ...
          setDisplayDate(
            new Date(
              newHighlight.getFullYear(),
              newHighlight.getMonth(),
              1
            )
          );
        }
  ...
  }, [highlightedDate, value, minDate, maxDate, setHighlightedDate, setDisplayDate, displayDate]);

  const handleMonthsNavigation = useCallback((e: React.KeyboardEvent) => {
  ...
  }, [highlightedMonth, value, displayDate, minDate, maxDate, setHighlightedMonth]);

  const handleYearsNavigation = useCallback((e: React.KeyboardEvent) => {
  ...
  const handleViewNavigation = useCallback((e: React.KeyboardEvent) => {
  ...
  }, [currentView, navigateViewDate, navigateYear, handleDaysNavigation, handleMonthsNavigation, handleYearsNavigation]);
  ...
  const handleInputKeyDown = useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    isOpen, currentView, highlightedDate, highlightedMonth, highlightedYear,
    onDateSelect, onMonthSelect, onYearSelect, openCalendar, closeCalendar, handleViewNavigation
  ]);
  ...
  const handleCalendarKeyDown = useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    currentView, highlightedDate, highlightedMonth, highlightedYear,
    onDateSelect, onMonthSelect, onYearSelect, closeCalendar,
    setCurrentView, setHighlightedDate, setHighlightedMonth, setHighlightedYear,
    displayDate, value, focusPortal, handleViewNavigation
  ]);
