...
const PresenceAvatar = memo(
  ({ user, isOnline, size, layoutIdPrefix, overlap }: PresenceAvatarProps) => {
    ...
    return (
      <Tooltip
        side="bottom"
        className={cn('shrink-0', overlap && sizeClasses[size].overlap)}
      >
        <TooltipTrigger asChild>
          ...
          >
            <Avatar
              className={cn('border-slate-200', sizeClasses[size].avatar)}
            >
              {profilePhotoUrl ? (
                <AvatarImage
                  src={profilePhotoUrl}
...
const UserPresenceAvatar = memo(
  ({
    ...
    const groupedUsers = useMemo(
      () => ({
        online: visibleUsers.filter(user =>
          onlineUserIds ? onlineUserIds.has(user.id) : true
        ),
        offline: visibleUsers.filter(
          user => onlineUserIds && !onlineUserIds.has(user.id)
        ),
      }),
        ...
        <TooltipProvider>
          ...
          >
            {groupedUsers.online.length > 0 ? (
              ...
              >
                <div
                  className={cn('flex items-center', sizeClasses[size].group)}
                >
