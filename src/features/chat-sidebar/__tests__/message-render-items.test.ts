...
describe("buildMessageRenderItems", () => {
  ...
  it("groups 3 consecutive image attachments into a single render item", () => {
  ...
  it("groups image attachments with the same reply target into a single render item", () => {
    const imageMessages = Array.from({ length: 2 }, (_, index) =>
      buildMessage({
        id: `image-${index + 1}`,
        message: `images/channel/reply-${index + 1}.png`,
        message_type: "image",
        file_name: `Reply-${index + 1}.png`,
        file_kind: undefined,
        file_mime_type: "image/png",
        file_storage_path: `images/channel/reply-${index + 1}.png`,
    ...
    expect(renderItems).toHaveLength(1);
    expect(renderItems[0]?.kind).toBe("image-group");
    expect(renderItems[0]?.messages.map((message) => message.id)).toEqual(["image-1", "image-2"]);
    expect(renderItems[0]?.anchorMessage.reply_to_id).toBe("source-message");
  });

  it("does not group image attachments with different reply targets", () => {
    const imageMessages = Array.from({ length: 2 }, (_, index) =>
      buildMessage({
        id: `image-${index + 1}`,
        message: `images/channel/reply-${index + 1}.png`,
        ...
        file_mime_type: "image/png",
        file_storage_path: `images/channel/reply-${index + 1}.png`,
        reply_to_id: `source-message-${index + 1}`,
        created_at: `2026-03-20T10:00:${String(index * 8).padStart(2, "0")}.000Z`,
      }),
    );
