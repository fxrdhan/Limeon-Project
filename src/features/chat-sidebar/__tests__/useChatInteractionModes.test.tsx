...
const { mockSearchConversationMessages, mockFetchConversationMessageContext } = vi.hoisted(() => ({
...
describe("useChatInteractionModes", () => {
  ...
  it("tracks search matches and navigates active search message", async () => {
    ...
    await waitFor(() => {
      expect(result.current.searchMatchedMessageIds).toEqual(["text-1", "file-1", "text-2"]);
    });
  ...
  it("clears the current selection without leaving selection mode", async () => {
    const messagesContainerRef = createRef<HTMLDivElement>();
    const messagesContainer = document.createElement("div");
    messagesContainer.scrollTo = vi.fn() as unknown as typeof messagesContainer.scrollTo;
    messagesContainerRef.current = messagesContainer;
    const selectionMessages = [
      buildMessage({
        id: "text-1",
  ...
  it("allows deleting selected incoming messages", async () => {
    ...
    act(() => {
      result.current.handleToggleMessageSelection("incoming-1");
    });

    expect(result.current.selectedVisibleMessages).toEqual(selectionMessages);
    expect(result.current.canDeleteSelectedMessages).toBe(true);
  });
  ...
  it("toggles grouped selections as a single bubble selection", async () => {
    ...
    act(() => {
      result.current.handleToggleMessageSelection(["image-1", "image-2", "image-3", "image-4"]);
    });
