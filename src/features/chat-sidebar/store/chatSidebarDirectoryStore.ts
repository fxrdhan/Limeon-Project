...
interface ChatSidebarDirectoryState {
...
export const useChatSidebarDirectoryStore = create<ChatSidebarDirectoryState>(
  (set, get) => ({
    ownerUserId: null,
    ...
    loadDirectoryPage: async (ownerUserId, reset = false) => {
      ...
      try {
        const { data, error } = await chatSidebarDirectoryGateway.getUsersPage(
          DIRECTORY_PAGE_SIZE,
