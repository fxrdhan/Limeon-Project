 ...
 */
export const useItemData = ({ formState, unitConversionHook }: UseItemDataProps) => {
  
  const fetchItemData = useCallback(async (id: string) => {
    try {
      ...
      const parsedConversionsFromDB = parseUnitConversions(itemData.unit_conversions);
      const mappedConversions = mapUnitConversions(parsedConversionsFromDB, formState.units);
      ...
      initializeUnitConversions(itemData, formState.units, unitConversionHook);
...
function parseUnitConversions(unitConversions: unknown): DBUnitConversion[] {
  ...
  try {
    const parsed = typeof unitConversions === "string"
 ...
 */
function mapUnitConversions(conversions: DBUnitConversion[], units: UnitData[]): UnitConversion[] {
  if (!Array.isArray(conversions)) return [];
  
  return conversions.map((conv: DBUnitConversion) => {
    const unitDetail = units.find((u) => u.id === conv.to_unit_id) ||
  ...
  for (const conv of conversions) {
    ...
    if (unitDetail && typeof conv.conversion_rate === "number") {
      ...
      const placeholderUnit: UnitData = {
        id: conv.to_unit_id || `temp_id_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
