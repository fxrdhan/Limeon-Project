...
const ALLOWED_ITEM_IMAGE_MIME_TYPES = new Set(["image/jpeg", "image/png", "image/webp"]);

const isTempImageUrl = (url: string) => url.startsWith("blob:") || url.startsWith("data:");
...
const buildHistoryImagePath = (itemId: string, slotIndex: number, file: File) => {
...
export const prepareItemData = async (
  ...
  const baseData = {
    ...
    is_level_pricing_active:
      formData.is_level_pricing_active !== undefined ? formData.is_level_pricing_active : true,
    ...
    measurement_denominator_unit_id: formData.measurement_denominator_unit_id || null,
...
const normalizeCustomerLevelDiscounts = (discounts?: CustomerLevelDiscount[]) => {
  ...
  const normalized = discounts
    ...
    .map((discount) => ({
      ...
      discount_percentage: Math.max(0, Number(discount.discount_percentage) || 0),
    }));
...
const syncCustomerLevelDiscounts = async (itemId: string, discounts?: CustomerLevelDiscount[]) => {
  if (!Array.isArray(discounts)) return;
...
export const checkExistingCodes = async (pattern: string): Promise<string[]> => {
  ...
  if (error) throw error;
  return data?.map((item) => item.code).filter((code): code is string => Boolean(code)) || [];
};
...
export const saveItemBusinessLogic = async ({
  ...
  const pendingImageUrls = Array.isArray(finalFormData.image_urls) ? finalFormData.image_urls : [];
  ...
  const resolveInventoryUnitId = async (conversion: {
    ...
    const candidateId = conversion.inventory_unit_id || conversion.to_unit_id || conversion.unit.id;

    if (candidateId.startsWith("dosage:") || conversion.unit.source_dosage_id) {
      const dosageId = conversion.unit.source_dosage_id || candidateId.replace(/^dosage:/, "");
      ...
      if (ensuredUnit.error || !ensuredUnit.data) {
        throw ensuredUnit.error || new Error("Gagal memastikan unit dari sediaan.");
      }
  ...
  const normalizedHierarchyEntries = [
    ...
    ...conversions.map((conversion) => ({
      ...
      parent_inventory_unit_id:
        ...
        resolvedBaseInventoryUnitId,
      contains_quantity:
        conversion.contains_quantity || conversion.factor_to_base || conversion.conversion_rate,
      factor_to_base: conversion.factor_to_base || conversion.conversion_rate || 1,
  ...
  if (isEditMode && itemId) {
    ...
    finalFormData.image_urls = pendingImageUrls.filter((url) => url && !isTempImageUrl(url));
    ...
    const { error: hierarchyError } = await itemDataService.replaceItemUnitHierarchy(
      itemId,
    ...
    await syncCustomerLevelDiscounts(itemId, finalFormData.customer_level_discounts);
    ...
    return {
      ...
      item: updatedItem ?? ({ id: itemId, ...itemUpdateData } as unknown as Item),
    };
  } else {
    // Create new item
    finalFormData.image_urls = pendingImageUrls.filter((url) => url && !isTempImageUrl(url));
    ...
    const { data: insertedItem, error: mainError } = await itemDataService.createItem(
      mainItemData as Record<string, unknown>,
    );
    ...
    await syncCustomerLevelDiscounts(insertedItem.id, finalFormData.customer_level_discounts);
    const { data: createdItem, error: createdItemError } = await itemsService.getItemWithDetails(
      insertedItem.id,
    );
    ...
    return {
      ...
      itemId: insertedItem.id,
      code: finalFormData.code,
      item: createdItem ?? ({ id: insertedItem.id, ...mainItemData } as unknown as Item),
    };
  }
};
...
export const saveEntityHelpers = {
  async saveCategory(categoryData: {
    ...
    const { data: newCategory, error } = await categoryService.create(categoryData);
  ...
  async saveUnit(unitData: { code?: string; name: string; description?: string }) {
  ...
  async saveDosage(dosageData: {
    ...
    const { data: newDosage, error } = await itemDosageService.create(dosageData);
  ...
  async saveManufacturer(manufacturerData: { code?: string; name: string; address?: string }) {
    const { data: newManufacturer, error } = await itemManufacturerService.create(manufacturerData);
