...
export const ItemTypeRules = {
  ...
  validate: (data: Partial<ItemType>): string[] => {
    ...
    if (!data.name?.trim()) {
      ...
      errors.push(`Nama jenis item maksimal ${ItemTypeRules.maxNameLength} karakter`);
    }
    
    if (data.description && data.description.length > ItemTypeRules.maxDescriptionLength) {
      errors.push(`Deskripsi maksimal ${ItemTypeRules.maxDescriptionLength} karakter`);
    }
