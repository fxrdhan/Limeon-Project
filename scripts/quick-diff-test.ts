...
function analyzeDiffModes(oldText: string, newText: string) {
  ...
  for (const mode of modes) {
    ...
    console.log(`${colors.bold}${mode.name} Mode:${colors.reset} (${executionTime.toFixed(2)}ms)`);
    console.log(`  Result: ${formatSegments(segments)}`);
    
    // Analyze segments
    const stats = analyzeSegments(segments);
    console.log(`  Stats: ${stats.total} segments (${stats.added} added, ${stats.removed} removed, ${stats.unchanged} unchanged)`);
  ...
  console.log(`  Old text match: ${oldMatch ? `${colors.green}✓${colors.reset}` : `${colors.red}✗${colors.reset}`}`);
  console.log(`  New text match: ${newMatch ? `${colors.green}✓${colors.reset}` : `${colors.red}✗${colors.reset}`}`);
...
function analyzeSegments(segments: DiffSegment[]) {
  ...
  if (smallSegments.length > 3) {
    stats.issues.push(`Possible over-fragmentation: ${smallSegments.length} segments ≤2 chars`);
  }
  ...
  if (alternatingCount > segments.length * 0.7) {
    stats.issues.push(`High alternating pattern: ${alternatingCount}/${segments.length} type changes`);
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (args.length === 0) {
    ...
    console.log(`Usage:`);
    console.log(`  yarn tsx scripts/quick-diff-test.ts "old text" "new text"  # Test specific pair`);
    console.log(`  yarn tsx scripts/quick-diff-test.ts --interactive         # Interactive mode`);
    console.log(`  yarn tsx scripts/quick-diff-test.ts                       # This help + interactive\n`);
    ...
    console.log(`${colors.red}Error: Please provide exactly two arguments (old text, new text) or use --interactive${colors.reset}`);
    console.log(`Usage: yarn tsx scripts/quick-diff-test.ts "old text" "new text"`);
    process.exit(1);
  }
}
