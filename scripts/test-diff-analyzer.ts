...
function checkFragmentation(segments: DiffSegment[]): string | null {
  ...
  if (
    alternatingCount > segments.length * 0.7 &&
    hasOnlySmallChanges &&
    !hasOnlyAdditions
  ) {
...
function runAllTests(): TestSummary {
  console.log(
    `${colors.bold}${colors.blue}🧪 Diff Analyzer Test Suite${colors.reset}\n`
  );
  ...
  console.log(
    `   Difficulty levels: ${Object.keys(stats.byDifficulty).length}\n`
  );
  ...
  for (const difficulty of Object.keys(stats.byDifficulty)) {
    const difficultyTests = results.filter(
      r => r.testCase.difficulty === difficulty
    );
    ...
    const status =
      result.passed === result.total
        ...
        : result.passed === 0
          ...
          : colors.yellow;
    console.log(
      `  ${category}: ${status}${result.passed}/${result.total}${colors.reset} (${percentage}%)`
    );
  }
    ...
    const status =
      result.passed === result.total
        ...
        : result.passed === 0
          ...
          : colors.yellow;
    console.log(
      `  ${difficulty}: ${status}${result.passed}/${result.total}${colors.reset} (${percentage}%)`
    );
  }
 ...
 */
function runSpecificTests(filter: {
  category?: string;
  difficulty?: string;
  id?: string;
}) {
  ...
  console.log(
    `${colors.blue}Running ${testCases.length} filtered test cases...${colors.reset}\n`
  );
  ...
  for (const testCase of testCases) {
    ...
    console.log(
      `   Category: ${testCase.category} | Difficulty: ${testCase.difficulty}`
    );
    console.log(
      `   Mode: expected ${testCase.expectedMode}, actual ${result.actualMode}`
    );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (args.length === 0) {
    ...
    console.log(
      `  Filter by category: yarn tsx scripts/test-diff-analyzer.ts --category abbreviation_expansion`
    );
    console.log(
      `  Filter by difficulty: yarn tsx scripts/test-diff-analyzer.ts --difficulty hard`
    );
    console.log(
      `  Run specific test: yarn tsx scripts/test-diff-analyzer.ts --id abbrev_001`
    );
  }
}
