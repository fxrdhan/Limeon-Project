...
fn parse_args() -> Result<Args, String> {
    ...
    while let Some(token) = iter.next() {
        ...
        match token.as_str() {
            ...
            "--mode" => args.mode = iter.next().ok_or("--mode requires a value")?,
...
fn print_help() {
    println!("ts_call_hierarchy");
    println!();
    println!("TypeScript/JavaScript Call Hierarchy via typescript-language-server (stdio).");
    println!();
    println!("Usage:");
    println!("  ts_call_hierarchy --file <path> --line <1-based> --character <1-based> [--root <path>]");
    println!(
        "  ts_call_hierarchy --file <path> --find <text> [--occurrence <n>] [--root <path>]"
    );
    println!();
    println!("Options:");
    println!("  --mode <incoming|outgoing|both|prepare>   Default: both");
    ...
    println!();
    println!("Output:");
    println!("  JSON with {{ prepare, incoming, outgoing }} (or subset based on mode).");
}
...
fn normalize_call_hierarchy(result: Value) -> Vec<Value> {
    match result {
        Value::Null => Vec::new(),
...
fn main() {
    ...
    let valid_mode = ["incoming", "outgoing", "both", "prepare"];
    if !valid_mode.iter().any(|mode| *mode == args.mode) {
    ...
    let prepare_result = match client.request("textDocument/prepareCallHierarchy", prepare_params) {
        Ok(result) => result,
        Err(err) => {
            eprintln!("{err}");
            client.shutdown();
    ...
    let prepare = normalize_call_hierarchy(prepare_result);

    let mut output = serde_json::Map::new();
    output.insert("prepare".to_string(), Value::Array(prepare.clone()));

    if args.mode == "prepare" {
        println!("{}", Value::Object(output).to_string());
        client.shutdown();
        return;
    }

    if prepare.is_empty() {
        output.insert("incoming".to_string(), Value::Array(vec![]));
        output.insert("outgoing".to_string(), Value::Array(vec![]));
        println!("{}", Value::Object(output).to_string());
        client.shutdown();
        return;
    }

    let first_item = prepare[0].clone();

    if args.mode == "incoming" || args.mode == "both" {
        match client.request("callHierarchy/incomingCalls", json!({"item": first_item})) {
            Ok(result) => {
                output.insert("incoming".to_string(), result);
            }
            Err(err) => {
                eprintln!("{err}");
                client.shutdown();
    ...
    if args.mode == "outgoing" || args.mode == "both" {
        match client.request("callHierarchy/outgoingCalls", json!({"item": first_item})) {
            Ok(result) => {
                output.insert("outgoing".to_string(), result);
            }
            Err(err) => {
                eprintln!("{err}");
                client.shutdown();
