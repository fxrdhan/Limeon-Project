...
export const resolveFileExtension = (
  ...
  const directExtension = sourceWithoutQuery.split(".").pop()?.trim().toLowerCase();
  ...
  if (mimeSubtype.includes("powerpoint") || mimeSubtype.includes("presentation")) {
    return "pptx";
  }
...
export const resolveChatMessageStoragePaths = (message: ChatStoragePathRecord) => {
  const storagePaths = new Set<string>();
  const primaryStoragePath =
    message.file_storage_path?.trim() || extractChatStoragePath(message.message);
