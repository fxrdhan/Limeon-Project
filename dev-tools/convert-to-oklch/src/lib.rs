...
fn is_always_ignored_dir(path: &Path) -> bool {
    ...
    ALWAYS_IGNORED_DIRS.contains(&file_name) || is_hidden_dir_name(file_name)
}

fn is_hidden_dir_name(file_name: &str) -> bool {
...
fn replace_colors_in_segment(content: &str, precision: u32) -> ReplacementResult {
...
fn replace_colors(content: &str, precision: u32) -> ReplacementResult {
    ...
    for line in content.split_inclusive('\n') {
        ...
        let skip_conversion = in_diff_fence && trimmed.starts_with('-');

        if skip_conversion {
            flush_replace_segment(&mut output, &mut segment, &mut conversions, precision);
        ...
        } else if let Some(marker) = markdown_fence_marker(trimmed) {
            fence_marker = Some(marker);
            in_diff_fence = markdown_fence_language(trimmed, marker).eq_ignore_ascii_case("diff");
        }
    }
...
fn flush_replace_segment(
    ...
    let result = replace_colors_in_segment(segment, precision);
...
fn markdown_fence_language<'a>(trimmed_line: &'a str, marker: &str) -> &'a str {
    trimmed_line
        .strip_prefix(marker)
        .unwrap_or_default()
        .trim_start()
