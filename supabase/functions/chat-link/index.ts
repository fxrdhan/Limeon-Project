...
import type {
  ChatSharedLinkCreateRequestPayload,
...
const createChatLinkRepository = ({
  ...
  async getAccessibleAttachmentByMessageId(messageId) {
    const { data, error } = await attachmentClient
      .from('chat_messages')
      .select(
        'id, sender_id, receiver_id, message_type, file_storage_path, shared_link_slug'
      )
      .eq('id', messageId)
  ...
  async getActiveSharedLinkByMessageId(messageId) {
    const { data, error } = await adminClient
      ...
      .select('id, slug, storage_path, message_id, revoked_at')
