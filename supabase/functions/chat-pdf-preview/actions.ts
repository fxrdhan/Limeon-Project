...
const hasPngSignature = (previewBytes: Uint8Array) =>
  PNG_SIGNATURE.every((byte, index) => previewBytes[index] === byte);
...
export const persistPdfPreview = async ({
  ...
  if (!hasPngSignature(previewBytes)) {
    return {
      status: 400,
  ...
  if (
    ...
    !isPdfPreviewableMessage(message)
  ) {
    return {
      status: 403,
