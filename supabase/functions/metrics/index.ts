...
async function calculateMetrics(supabase) {
  const { data: metrics, error } = await supabase.from('api_metrics').select('*').order('timestamp', {
    ascending: false
  });
  if (error) {
  ...
  const averageResponseSize = metricsWithResponse.length > 0 ? totalResponseSize / metricsWithResponse.length : 0;
...
serve(async (req) => {
  ...
  try {
    ...
    if (method === 'GET' && url.pathname.endsWith('/metrics/recent')) {
      ...
      try {
        const { data: recentRequests, error } = await supabase.from('api_metrics').select('*').order('timestamp', {
          ascending: false
        }).limit(limit);
        if (error) {
          throw error;
